...
public class ClaimApplicationFlowServiceImpl implements ClaimApplicationFlowService {
    ...
    public ApiResponseDTO<GeneralClaimResponse> create(GeneralClaimCreateRequest request) {
        ...
        .build();
        List<ClaimApplicationWorkflowResponseDto> workflowDetails = claimApplicationWorkflowService.create(claimApplication, workflowRequest);
        ...
        response.setForfeitedComponents(
                forfeitedComponents.stream().map(claimApplicationForfeitedComponentResponseMapper::toResponse)
                        .toList());
    ...
    private GeneralClaimResponse buildGeneralClaimResponse(
        ...
        response.setDeductionDetail(
                firstOrNull(claimApplication.getDeductionDetails()) == null
                        ? null
                        : claimApplicationDeductionResponseMapper.toResponse(
                                firstOrNull(claimApplication.getDeductionDetails())));

        response.setCalculationSummary(
                firstOrNull(claimApplication.getCalculationSummaries()) == null
                        ? null
                        : claimApplicationCalculationSummaryResponseMapper.toResponse(
                                firstOrNull(claimApplication.getCalculationSummaries())));
    ...
    @Override
@Transactional(readOnly = true)
public ApiResponseDTO<GeneralClaimResponse> findByApplicationId(Long applicationId) {
    ...
    ClaimApplication claimApplication =
            claimApplicationService.getById(applicationId);

    GeneralClaimResponse response =
            buildGeneralClaimResponse(claimApplication);
