...
@Entity
@Table(
        name = "CLAIM_APPLICATION_DEDUCTION_DETAIL",
...
public class ClaimApplicationDeductionDetail {
    ...
    @JoinColumn(
            ...
            nullable = false,
            foreignKey = @ForeignKey(name = "FK_CADD_DED_CLAIM_APP")
    )
    ...
    @JoinColumn(
            ...
            foreignKey = @ForeignKey(name = "FK_CADD_DED_TYPE")
    )
    private DeductionTypeMaster deductionType;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "DEDUCTION_REFERENCE_TYPE_ID",
            foreignKey = @ForeignKey(name = "FK_CADD_DED_REF_TYPE")
    )
    ...
    @JoinColumn(
            ...
            foreignKey = @ForeignKey(name = "FK_CADD_DED_REVIEW_STATUS")
    )
