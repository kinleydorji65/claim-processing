...
public class ClaimApplicationVerification {
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "VERIFICATION_DECISION_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_VER_DECISION")
    )
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "MEMBER_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_MEMBER_REVIEW_STATUS")
    )
    private ReviewStatusMaster memberReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "BANK_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_BANK_REVIEW_STATUS")
    )
    private ReviewStatusMaster bankReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "DOCUMENT_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_DOC_REVIEW_STATUS")
    )
    private ReviewStatusMaster documentReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "CONTRIBUTION_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_CONTRIB_REVIEW_STATUS")
    )
    private ReviewStatusMaster contributionReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "RULE_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_RULE_REVIEW_STATUS")
    )
    ...
    @JoinColumn(
            ...
            foreignKey = @ForeignKey(name = "FK_CAV_LOAN_REVIEW_STATUS")
    )
    private ReviewStatusMaster loanReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "DEDUCTION_REVIEW_STATUS_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_DEDUCTION_REVIEW_STATUS")
    )
    private ReviewStatusMaster deductionReviewStatus;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "FINAL_VERIFICATION_DECISION_ID",
            foreignKey = @ForeignKey(name = "FK_CAV_FINAL_VER_DECISION")
    )
